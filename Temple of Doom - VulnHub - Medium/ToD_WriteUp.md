...
It turns to be a NodeJs application. If we interact with the server a bit, information disclosure reveals that data is being *deserialized* on the backend of the server through the *unserialize()* function.
...
This function it is extremely vulnerable, allowing an attacker to gain unauthenticated remote code execution. A carefully crafted payload encoded in the correct way will do the trick.
...
So the plan is just to run the suid in the context of the owner with 
...
Enumerating from the new user, we find out that we can run an interesting sudo command: 
